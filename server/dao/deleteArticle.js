...
function deleteArticle(gro_num, art_id, succes) {
    ...
    var getGroList = "select gro_article_list from `group` where gro_num = ?"
    var updateGroArtList = "update `group` set gro_article_list = ? where gro_num = ?"
        ...
        if(error == null){
            ...
            connection.query(getGroList, gro_num, function (error, result) {
                ...
                if(error == null){
                    ...
                    connection.query(updateGroArtList, [artList, gro_num],  (error, result)=>{
                        // 更新文章列表
                        if(error == null){
                            console.log(result)
